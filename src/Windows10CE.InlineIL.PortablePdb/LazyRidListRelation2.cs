...
namespace Windows10CE.InlineIL.PortablePdb;
    ...
    internal class LazyRidListRelation2<TAssociationRow>
        where TAssociationRow : struct, IMetadataRow
    {
        ...
        private readonly MetadataDirectory _memberMetadata;
        private readonly MetadataDirectory _memberOwnerMetadata;
        ...
        public LazyRidListRelation2(
            MetadataDirectory memberMetadata,
            MetadataDirectory memberOwnerMetadata,
            TableIndex memberTable,
            TableIndex associationTable,
            GetOwnerRidDelegate getOwnerRid,
            GetMemberListDelegate getMemberList)
        {
            _memberMetadata = memberMetadata ?? throw new ArgumentNullException(nameof(memberMetadata));
            _memberOwnerMetadata = memberOwnerMetadata ?? throw new ArgumentNullException(nameof(memberOwnerMetadata));
        ...
        private void Initialize()
        {
            lock (_lock)
            {
                ...
                var memberTablesStream = _memberMetadata.GetStream<TablesStream>();
                var memberOwnerTablesStream = _memberOwnerMetadata.GetStream<TablesStream>();
                var associationTable = memberTablesStream.GetTable<TAssociationRow>(_associationTable);
                var memberTable = memberOwnerTablesStream.GetTable(_memberTable);
